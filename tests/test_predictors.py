...
from paa_sampling_free.modeling.backbone import build_backbone # NoQA
from paa_sampling_free.modeling.roi_heads.roi_heads import build_roi_heads # NoQA
from paa_sampling_free.modeling import registry
from paa_sampling_free.config import cfg as g_cfg
...
class TestPredictors(unittest.TestCase):
    def test_roi_box_predictors(self):
        ...
        for cur_in, cur_out, cur_cfg in _test_predictors(
            ...
            ssampling_frees, bbox_deltas = cur_out[0], cur_out[1]
            self.assertEqual(
                ssampling_frees.shape[1], cur_cfg.MODEL.ROI_BOX_HEAD.NUM_CLASSES)
            self.assertEqual(ssampling_frees.shape[0], cur_in.shape[0])
            self.assertEqual(ssampling_frees.shape[0], bbox_deltas.shape[0])
            self.assertEqual(ssampling_frees.shape[1] * 4, bbox_deltas.shape[1])
