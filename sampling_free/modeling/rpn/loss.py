...
class RPNLossComputation(torch.nn.Module):
    ...
    def __call__(self, anchors, objectness, box_regression, targets):
        ...
        if self.sampling_free:
            ...
            box_loss = 0.5 * smooth_l1_loss(
                box_regression[positive],
                regression_targets[positive],
                beta=1.0 / 9,
                size_average=True,
            ) 
            objectness = objectness.view(-1, 1)
            labels = labels.view(-1, 1)
            objectness_loss = self.ce_loss(objectness[valid], labels[valid].int()) / positive.sum()

            with torch.no_grad():
                r = box_loss / objectness_loss
            objectness_loss = r * objectness_loss
        ...
        return dict(objectness_loss=objectness_loss, rpn_box_loss=box_loss)
...
def make_rpn_loss_evaluator(cfg, box_coder):
    loss_evaluator = RPNLossComputation(
        cfg, box_coder, generate_rpn_labels
    )
