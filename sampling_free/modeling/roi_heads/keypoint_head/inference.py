...
class KeypointPostProcessor(nn.Module):
    ...
    def forward(self, x, boxes):
        ...
        ssampling_frees = None
        if self.keypointer:
            mask_prob, ssampling_frees = self.keypointer(x, boxes)

        assert len(boxes) == 1, "Only non-batched inference supported for now"
        boxes_per_image = [box.bbox.size(0) for box in boxes]
        mask_prob = mask_prob.split(boxes_per_image, dim=0)
        ssampling_frees = ssampling_frees.split(boxes_per_image, dim=0)

        results = []
        for prob, box, ssampling_free in zip(mask_prob, boxes, ssampling_frees):
            bbox = BoxList(box.bbox, box.size, mode="xyxy")
            for field in box.fields():
                bbox.add_field(field, box.get_field(field))
            prob = PersonKeypoints(prob, box.size)
            prob.add_field("logits", ssampling_free)
...
def heatmaps_to_keypoints(maps, rois):
    ...
    end_ssampling_frees = np.zeros((len(rois), num_keypoints), dtype=np.float32)
    for i in range(len(rois)):
        ...
        # roi_map_probs = ssampling_frees_to_probs(roi_map.copy())
        w = roi_map.shape[2]
        ...
        xy_preds[i, 2, :] = 1
        end_ssampling_frees[i, :] = roi_map[np.arange(num_keypoints), y_int, x_int]

    return np.transpose(xy_preds, [0, 2, 1]), end_ssampling_frees
...
class Keypointer(object):
    ...
    def __call__(self, masks, boxes):
        ...
        result, ssampling_frees = heatmaps_to_keypoints(
            masks.detach().cpu().numpy(), boxes[0].bbox.cpu().numpy()
        )
        return torch.from_numpy(result).to(masks.device), torch.as_tensor(ssampling_frees, device=masks.device)
