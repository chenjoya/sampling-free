...
from sampling_free import layers
from sampling_free.modeling import registry
...
class KeypointRCNNPredictor(nn.Module):
    def __init__(self, cfg, in_channels):
        ...
        deconv_kernel = 4
        self.kps_ssampling_free_lowres = layers.ConvTranspose2d(
            input_features,
            num_keypoints,
            deconv_kernel,
            stride=2,
        ...
        nn.init.kaiming_normal_(
            self.kps_ssampling_free_lowres.weight, mode="fan_out", nonlinearity="relu"
        )
        nn.init.constant_(self.kps_ssampling_free_lowres.bias, 0)
        self.up_scale = 2
        self.out_channels = num_keypoints

    def forward(self, x):
        x = self.kps_ssampling_free_lowres(x)
