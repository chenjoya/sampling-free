...
def im_detect_bbox_aug_vote(model, images, device):
    ...
    for _, boxlist_ts in enumerate(boxlists_ts):
        ...
        ssampling_frees = torch.cat(
            [boxlist_t.get_field('ssampling_frees') for boxlist_t in boxlist_ts])
        labels = torch.cat(
            [boxlist_t.get_field('labels') for boxlist_t in boxlist_ts])
        boxlist = BoxList(bbox, boxlist_ts[0].size, boxlist_ts[0].mode)
        boxlist.add_field('ssampling_frees', ssampling_frees)
...
def merge_result_from_multi_scales(boxlists, nms_type='nms', vote_thresh=0.65):
    ...
    for i in range(num_images):
        ...
        for j in range(1, cfg.MODEL.RETINANET.NUM_CLASSES):
            ...
            boxlist_for_class = boxlist_nms(boxlist_for_class, cfg.MODEL.ATSS.NMS_TH, ssampling_free_field="ssampling_frees",
                                            nms_type=nms_type, vote_thresh=vote_thresh)
            num_labels = len(boxlist_for_class)
            boxlist_for_class.add_field("labels", torch.full((num_labels,), j, dtype=torch.int64, device=ssampling_frees.device))
...
def boxlist_nms(boxlist, nms_thresh, max_proposals=-1, ssampling_free_field="ssampling_frees", nms_type='nms', vote_thresh=0.65):
    if nms_thresh <= 0:
        return boxlist
    mode = boxlist.mode
    boxlist = boxlist.convert("xyxy")
    boxes = boxlist.bbox
    ssampling_free = boxlist.get_field(ssampling_free_field)
    if nms_type == 'nms':
        keep = _box_nms(boxes, ssampling_free, nms_thresh)
        if max_proposals > 0:
            keep = keep[: max_proposals]
        boxlist = boxlist[keep]
    else:
        if nms_type == 'vote':
            boxes_vote, ssampling_frees_vote = bbox_vote(boxes, ssampling_free, vote_thresh)
        else:
            boxes_vote, ssampling_frees_vote = soft_bbox_vote(boxes, ssampling_free, vote_thresh)
        if len(boxes_vote) > 0:
            boxlist.bbox = boxes_vote
            boxlist.extra_fields['ssampling_frees'] = ssampling_frees_vote
    return boxlist.convert(mode)


def bbox_vote(boxes, ssampling_frees, vote_thresh):
    boxes = boxes.cpu().numpy()
    ssampling_frees = ssampling_frees.cpu().numpy().reshape(-1, 1)
    det = np.concatenate((boxes, ssampling_frees), axis=1)
    if det.shape[0] <= 1:
    ...
    while det.shape[0] > 0:
        ...
        if merge_index.shape[0] <= 1:
            ...
            max_ssampling_free = np.max(det_accu[:, 4])
            det_accu_sum = np.zeros((1, 5))
            det_accu_sum[:, 0:4] = np.sum(det_accu[:, 0:4], axis=0) / np.sum(det_accu[:, -1:])
            det_accu_sum[:, 4] = max_ssampling_free
            try:
    ...
    ssampling_frees = torch.from_numpy(dets[:, 4]).float().cuda()

    return boxes, ssampling_frees


def soft_bbox_vote(boxes, ssampling_frees, vote_thresh):
    boxes = boxes.cpu().numpy()
    ssampling_frees = ssampling_frees.cpu().numpy().reshape(-1, 1)
    det = np.concatenate((boxes, ssampling_frees), axis=1)
    if det.shape[0] <= 1:
